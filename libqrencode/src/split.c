...
static int Split_eatNum(const char *string, QRinput *input,QRencodeMode hint)
{
	...
	if(mode == QR_MODE_8) {
		dif = QRinput_estimateBitsModeNum((int)run) + 4 + ln
			+ QRinput_estimateBitsMode8(1) /* + 4 + l8 */
			- QRinput_estimateBitsMode8((int)(run + 1)) /* - 4 - l8 */;
		if(dif > 0) {
	...
	if(mode == QR_MODE_AN) {
		dif = QRinput_estimateBitsModeNum((int)run) + 4 + ln
			...
			- QRinput_estimateBitsModeAn((int)(run + 1)) /* - 4 - la */;
		if(dif > 0) {
			return Split_eatAn(string, input, hint);
		}
	}

	ret = QRinput_append(input, QR_MODE_NUM, (int)run, (unsigned char *)string);
	if(ret < 0) return -1;
...
static int Split_eatAn(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	if(*p && !isalnum(*p)) {
		dif = QRinput_estimateBitsModeAn((int)run) + 4 + la
			+ QRinput_estimateBitsMode8(1) /* + 4 + l8 */
			- QRinput_estimateBitsMode8((int)(run + 1)) /* - 4 - l8 */;
		if(dif > 0) {
			return Split_eat8(string, input, hint);
	...
	ret = QRinput_append(input, QR_MODE_AN, (int)run, (unsigned char *)string);
	if(ret < 0) return -1;
...
static int Split_eatKanji(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	run = p - string;
	ret = QRinput_append(input, QR_MODE_KANJI, (int)run, (unsigned char *)string);
	if(ret < 0) return -1;
...
static int Split_eat8(const char *string, QRinput *input, QRencodeMode hint)
{
	...
	run = p - string;
	ret = QRinput_append(input, QR_MODE_8, (int)run, (unsigned char *)string);
	if(ret < 0) return -1;
